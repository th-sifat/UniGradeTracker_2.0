...
// Fail fast with a clear message if required DB env vars are missing (common on Vercel)
if (!process.env.DATABASE_URL) {
  console.error('FATAL: DATABASE_URL environment variable is not set. Set it in Vercel Project Settings → Environment Variables.');
}
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    // Surface common deployment misconfigurations clearly to the client
    if (msg.includes('Environment variable not found') || msg.includes('DATABASE_URL') || !process.env.DATABASE_URL) {
      return res.status(500).json({
        error: 'Database not configured. Please set DATABASE_URL (and DIRECT_URL) environment variables in Vercel and redeploy.'
      });
    }
